...
No estoy seguro de que sea la mejor solución, pero es la manera que hace un tiempo nos permitió al equipo de MG automatizar reportes y realizar ETLs que con herramientas propias de AWS (como GLUE) no podíamos resolver.
...
Como explica [@_ColinFay](https://twitter.com/_ColinFay) en su [introducción a Docker para usarixs de R](https://colinfay.me/docker-r-reproducibility/), Docker es un programa que realiza virtualizaciones de sistemas operativos que nos permite trabajar con diferentes OS a nivel local y también extrapolarlos para que evitemos el "en mi máquina funciona".

Para poder trabajar vamos a necesitar una imagen que es la definición del sistema operativo y un container que es lo que nos permite ejecutar la instancia.


####¿Es importante el script de este ejemplo? 

No. Como es un ejemplo, el contenido del script no tiene otro fin más que mostrar cómo funciona la automatización. Vamos a usar un *.Rmd*